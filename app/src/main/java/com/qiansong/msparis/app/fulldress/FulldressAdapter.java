 ...
 * 礼服首页adapter
 */
public class FulldressAdapter extends RecyclerView.Adapter<FulldressAdapter.ViewHolder> {

    private Context context;
    private List<ProductBean.DataEntity.RowsEntity> data;

    public FulldressAdapter(Context context, List<ProductBean.DataEntity.RowsEntity> data) {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ExclusiveUtils.setImageUrl(holder.skuIv,data.get(position).getCover_image(),0);
        holder.skuNameTv.setText(data.get(position).getName());
        holder.xiandingTv.setText(data.get(position).getLimit_tag());
        holder.skuPrice01Tv.setText((data.get(position).getRental_price()/100)+"/3天");
        holder.skuPrice02Tv.setText("市场价:"+(data.get(position).getMarket_price()/100));
...
//                                ContentUtil.makeToast(INSTANCE,dataEntity.getIs_favorite()==0?"收藏成功":"取消收藏");
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra(GlobalConsts.INIT_DATA,new String[]{data.get(position).getId(),"2"});
